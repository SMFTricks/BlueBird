...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		while ($message = $context['get_pmessage']('message'))
		{
			...
			echo '
	<div class="', $window_class, ' post_wrapper" style="margin-top: 0;">
		<div class="date_post"><div>', $txt['from'], ' ', ($context['from_or_to'] == 'from' ? $message['member']['link'] : (empty($message['recipients']['to']) ? '' : implode(', ', $message['recipients']['to']))), ' ', $txt['on'], ' ', $message['time'], '</div><span>';if (!empty($message['is_replied_to'])) echo ' &#171; ', $txt['pm_is_replied_to'], ' &#187;&nbsp;|&nbsp;'; echo ' ', $message['subject'], '</span></div>
		<div class="poster">
		...
		if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
				echo '
					<img src="', $message['member']['online']['image_href'], '" title="', $message['member']['online']['text'], '" alt="', $message['member']['online']['text'], '" />';
				echo '
					', $message['member']['link'], '
				</h4>
		...
		if (!empty($message['member']['group']) && ($context['can_send_pm']))
			echo '
		...
		if ((empty($message['member']['group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
			...
			echo '
				...
				echo '
				<li class="stars"><hr class="divisorp" />', $message['member']['group_stars'], '</li>';
				
				// Show the member's custom title, if they have one.
				if (!empty($message['member']['title']))
					echo '
				<li class="title"><hr class="divisorp" />', $message['member']['title'], '</li>';

				// Show how many posts they have made.
				if (!isset($context['disabled_fields']['posts']))
					echo '
				<li class="postcount"><hr class="divisorp" />', $txt['member_postcount'], ': ', $message['member']['posts'], '</li>';

				// Is karma display enabled?  Total or +/-?
				if ($modSettings['karmaMode'] == '1')
					echo '
				<li class="karma"><hr class="divisorp" />', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</li>';
				...
				if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
					echo '
				<li class="gender"><hr class="divisorp" />', $txt['gender'], ': ', $message['member']['gender']['image'], '</li>';

				// Show their personal text?
				if (!empty($settings['show_blurb']) && $message['member']['blurb'] != '')
					echo '
				<li class="blurb"><hr class="divisorp" />', $message['member']['blurb'], '</li>';
				...
				if (!empty($message['member']['custom_fields']))
				{
					...
					foreach ($message['member']['custom_fields'] as $custom)
					{
						...
						if (empty($shown))
						{
							$shown = true;
							echo '
				<li class="im_icons"><hr class="divisorp" />
					<ul>';
						}
						echo '
						<li>', $custom['value'], '</li>';
				...
				if (!empty($message['member']['custom_fields']))
				{
					foreach ($message['member']['custom_fields'] as $custom)
						if (empty($custom['placement']) || empty($custom['value']))
							echo '
				<li class="custom"><hr class="divisorp" />', $custom['title'], ': ', $custom['value'], '</li>';
				}

				// Are we showing the warning status?
				if ($message['member']['can_see_warning'])
				echo '
				<li class="warning"><hr class="divisorp" />', $context['can_issue_warning'] ? '<a href="' . $scripturl . '?action=profile;area=issuewarning;u=' . $message['member']['id'] . '">' : '', '<img src="', $settings['images_url'], '/warning_', $message['member']['warning_status'], '.gif" alt="', $txt['user_warn_' . $message['member']['warning_status']], '" />', $context['can_issue_warning'] ? '</a>' : '', '<span class="warn_', $message['member']['warning_status'], '">', $txt['warn_' . $message['member']['warning_status']], '</span></li>';
			}
